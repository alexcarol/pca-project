...
/************/
	...
	if ((Angles.z_twist = (int *)malloc(MAX_ROTATIONS * sizeof(int))) &&
	    (Angles.theta = (int *)malloc(MAX_ROTATIONS * sizeof(int))) && (Angles.phi = (int *)malloc(MAX_ROTATIONS * sizeof(int)))) {
	} else {
...
/************/
	...
	for (theta = angle_step; theta < 180; theta += angle_step) {
		...
		phi_step_for_this_theta =
		    57.29578 * acos((cos(0.017453293 * angle_step) - (cos(0.017453293 * theta) * cos(0.017453293 * theta))) /
				    (sin(0.017453293 * theta) * sin(0.017453293 * theta)));

		while ((360 % phi_step_for_this_theta) != 0)
			phi_step_for_this_theta--;
...
/************/
	...
	if ((Angles.z_twist = (int *)realloc(Angles.z_twist, (1 + n) * sizeof(int))) &&
	    (Angles.theta = (int *)realloc(Angles.theta, (1 + n) * sizeof(int))) && (Angles.phi = (int *)realloc(Angles.phi, (1 + n) * sizeof(int)))) {
	} else {
	GENERAL_MEMORY_PROBLEM}
...
/************/
	...
	if (angle_range != 0)
		angle_range = angle_step * (int)(angle_range / angle_step);

/************/

	phi_step_for_this_theta =
	    57.29578 * acos((cos(0.017453293 * angle_step) - (cos(0.017453293 * theta) * cos(0.017453293 * theta))) / (sin(0.017453293 * theta) * sin(0.017453293 * theta)));
...
/************/
	...
	for (z_twist_step = z_twist - angle_range; z_twist_step <= z_twist + angle_range; z_twist_step += angle_step) {
		...
		if (this_z_twist_step >= 360)
			this_z_twist_step -= 360;
		if (this_z_twist_step < 0)
			this_z_twist_step += 360;
