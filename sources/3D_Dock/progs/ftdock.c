...
/************/
	...
	if (((output_file_name = (char *)malloc(500 * sizeof(char))) == NULL) ||
	    ((static_file_name = (char *)malloc(500 * sizeof(char))) == NULL) || ((mobile_file_name = (char *)malloc(500 * sizeof(char))) == NULL)) {
...
/************/
	...
	if (rescue == 1) {
		...
		while (fgets(line_buffer, 99, ftdock_file)) {
			...
			if (strncmp(line_buffer, "Output file name", 16) == 0)
				sscanf(line_buffer, "Output file name :: %s", output_file_name);
			if (strncmp(line_buffer, "Global grid size", 16) == 0)
				sscanf(line_buffer, "Global grid size :: %d", &global_grid_size);
			if (strncmp(line_buffer, "Global search angle step", 24) == 0)
				sscanf(line_buffer, "Global search angle step :: %d", &angle_step);
			if (strncmp(line_buffer, "Global surface thickness", 24) == 0)
				sscanf(line_buffer, "Global surface thickness :: %f", &surface);
			if (strncmp(line_buffer, "Global internal deterrent value", 31) == 0)
				sscanf(line_buffer, "Global internal deterrent value :: %f", &internal_value);
			if (strncmp(line_buffer, "Electrostatics                     ::     on", 44) == 0)
				electrostatics = 1;
			if (strncmp(line_buffer, "Electrostatics                     ::    off", 44) == 0)
				electrostatics = 0;
			if (strncmp(line_buffer, "Global keep per rotation", 25) == 0)
				sscanf(line_buffer, "Global keep per rotation :: %d", &keep_per_rotation);
...
/************/
	...
	if (((static_grid = (fftw_real *) malloc(global_grid_size * global_grid_size * (2 * (global_grid_size / 2 + 1)) * sizeof(fftw_real))) == NULL)
	    || ((mobile_grid = (fftw_real *) malloc(global_grid_size * global_grid_size * (2 * (global_grid_size / 2 + 1)) * sizeof(fftw_real))) == NULL)
	    || ((convoluted_grid = (fftw_real *) malloc(global_grid_size * global_grid_size * (2 * (global_grid_size / 2 + 1)) * sizeof(fftw_real))) == NULL)
	    ) {
	...
	if (electrostatics == 1) {
		...
		if (((static_elec_grid = (fftw_real *) malloc(global_grid_size * global_grid_size * (2 * (global_grid_size / 2 + 1)) * sizeof(fftw_real))) == NULL)
		    || ((mobile_elec_grid = (fftw_real *) malloc(global_grid_size * global_grid_size * (2 * (global_grid_size / 2 + 1)) * sizeof(fftw_real))) == NULL)
		    || ((convoluted_elec_grid = (fftw_real *) malloc(global_grid_size * global_grid_size * (2 * (global_grid_size / 2 + 1)) * sizeof(fftw_real))) == NULL)
		    ) {
		...
		for (fx = 0; fx < global_grid_size; fx++) {
			for (fy = 0; fy < global_grid_size; fy++) {
				for (fz = 0; fz < global_grid_size / 2 + 1; fz++) {
					...
					multiple_fsg[fxyz].re = static_fsg[fxyz].re * mobile_fsg[fxyz].re + static_fsg[fxyz].im * mobile_fsg[fxyz].im;
					multiple_fsg[fxyz].im = static_fsg[fxyz].im * mobile_fsg[fxyz].re - static_fsg[fxyz].re * mobile_fsg[fxyz].im;
...
/************/
		...
		for (x = 0; x < global_grid_size; x++) {
			fx = x;
			if (fx > (global_grid_size / 2))
				fx -= global_grid_size;

			for (y = 0; y < global_grid_size; y++) {
				fy = y;
				if (fy > (global_grid_size / 2))
					fy -= global_grid_size;

				for (z = 0; z < global_grid_size; z++) {
					fz = z;
					if (fz > (global_grid_size / 2))
						fz -= global_grid_size;
...
/************/
	...
	while (fgets(line_buffer, 99, ftdock_file)) {
		...
		sscanf(line_buffer, "G_DATA %d %d %d %f  %d %d %d  %d %d %d", &id, &id2, &SCscore, &RPscore, &x, &y, &z, &z_twist, &theta, &phi);
	...
	if (electrostatics == 1) {
		...
		for (i = 0; i <= min(kept_scores, (NUMBER_TO_KEEP - 1)); i++) {
			...
			fprintf(ftdock_file, "G_DATA %6d   %6d    %7d       %8.3f      %4d %4d %4d      %4d%4d%4d\n",
				...
				Scores[i].coord[1], Scores[i].coord[2], Scores[i].coord[3], Scores[i].angle[1], Scores[i].angle[2], Scores[i].angle[3]);
